...
| 多仓库索引维护 | `workspace status/sync/check/rebuild/vacuum`（基于全局注册表批量巡检与维护） |
| 未解析引用警告 | `scan` / `sync` 输出 |

`scan` 会把每个工作区自动注册到全局注册表（`~/.apsgraph/registry.json`），`workbench` 在单一本地服务内提供所有已注册工作区的查询与切换，服务一次扫描多个仓库的模型资产；`workspace` 命令族基于同一注册表提供批量的新鲜度巡检、增量同步、健康检查与索引维护。
...
支持 MySQL、Oracle、PostgreSQL 方言，以及 MySQL 家族分布式数据库 TDSQL、GoldenDB：全部方言支持 RANGE 按日分区（转换函数按分区键类型与方言自动选择，起始/终止日期 yyyymmdd 输入，支持基准日期+粒度 D/M/Y+前置/后置分区数的快捷推算，终止留空以 MAXVALUE 兜底）；workbench 的生成 DDL 弹窗可为分布式目标选择表分片类型（分片表/广播表/复制表，分片表按唯一索引归属提示选择分片键）。所有 DDL 均只生成，不执行。
