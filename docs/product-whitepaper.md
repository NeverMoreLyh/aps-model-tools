...
支持 MySQL、Oracle、PostgreSQL 方言，以及 MySQL 家族分布式数据库 TDSQL、GoldenDB：workbench 的生成 DDL 弹窗可为分布式目标选择表分片类型（分片表/广播表/复制表，分片表按唯一索引归属提示选择分片键）并生成 RANGE 按日分区（转换函数按分区键类型自动选择，起始/终止日期 yyyymmdd 输入，终止留空以 MAXVALUE 兜底）。所有 DDL 均只生成，不执行。
