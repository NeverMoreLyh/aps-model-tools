...
- 布局交互：左侧菜单一二级聚合，一级菜单为交易、服务、表、字典数据项、枚举类型、基础类型，其余页面统一归入可展开的“其他”分组（切到其中页面时自动展开）；左侧菜单可收起/展开；结果列表默认占窗口宽度 1/4 且可通过分隔条拖拽调节；mermaid 流程图画布默认 0.6 缩放，支持放大/缩小/重置/全屏、滚轮缩放与拖拽平移；全屏弹层必须提供关闭按钮。
