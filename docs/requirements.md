...
- 支持按顶层模型查询并展示递归包含的子模型树（懒加载）；支持表、服务、交易 flowtran、批量交易、复合类型、数据字典、枚举类型、错误码的独立查询页；基础类型为内置 APS SimpleType 清单页。
- 每个查询页提供查询输入框和查询类型（维度），支持按 `id`、`fullId`、`longname`、`desc` 单维度或全部维度模糊搜索，结果分页展示。
- 详情面板按类型结构化展示：表展示字段、物理索引、ODB 索引、序列；服务展示各服务操作的输入/输出；交易展示输入、输出与流程编排；批量交易展示输入字段、批量步骤与步骤组；字典/错误码展示数据项与枚举值明细。
- 已解析引用必须可点击跳转到目标详情；未解析引用显示原始目标。
- 数据字典与错误码同为 DICTIONARY 节点，必须按来源文件后缀 `.d_schema.xml` / `.error.xml` 区分。
