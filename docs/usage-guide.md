...
- **多方言 DDL**：MySQL / Oracle / PostgreSQL / TDSQL / GoldenDB 建表脚本生成，分布式方言支持表分片类型（分片表/广播表/复制表）与 RANGE 按日分区
