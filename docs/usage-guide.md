...
- **多方言 DDL**：MySQL / Oracle / PostgreSQL / TDSQL / GoldenDB 建表脚本生成，全部方言支持 RANGE 按日分区；分布式方言另支持表分片类型（分片表/广播表/复制表）
...
| `ddl` | 单表 DDL 预览（实验性） |
| `ddl-gen` | 批量生成 MySQL/Oracle/PostgreSQL/TDSQL/GoldenDB DDL |
...
| 参数 | 说明 |
|---|---|
| `--dialect` | `mysql` / `oracle` / `postgresql` / `tdsql` / `goldendb`（默认 mysql；tdsql/goldendb 为 MySQL 家族分布式方言，CLI 不带分片/分区参数时按单表生成） |
...
8. **详情面板**：按类型结构化展示，表格列按元数据模型对象定义——交易/服务的输入输出、复合类型与数据字典的数据项表头为 `字典ID, 字段, 中文名, 类型, 必填, 多值, 默认值, 固定值, 描述, 别名`；表的字段表头为 `字典ID, 字段, DbName, 中文名, 类型, 可为空, 默认值, 描述, 是否主键`。表另提供“生成 DDL”按钮：弹窗选择 MySQL / Oracle / PostgreSQL / TDSQL / GoldenDB 后生成建表语句（只生成、不执行），支持一键复制；选择 TDSQL / GoldenDB 分布式方言时弹窗出现“表分片类型”选择（单表/分片表/广播表——GoldenDB 语境显示为单节点存储表/复制表）：选择分片表后出现分片键下拉框，选项标注该字段所属的主键与唯一索引，若存在未包含分片键的唯一索引（含主键）则显示约束警示（TDSQL 要求唯一索引必须包含分片键、GoldenDB 要求分片键必须在主键中），选择 GoldenDB 时还提供节点组输入框（默认 `g1,g2,g3,g4`）；弹窗另有“创建分区表”勾选项（全部方言可用）：勾选后选择分区键（选项按字段类型标注转换函数：日期类型 `TO_DAYS`、时间戳 `UNIX_TIMESTAMP`、yyyymmdd 字符串 `RANGE COLUMNS`、整数直接列——MySQL 家族专用，Oracle 用 `TO_DATE`/`TO_TIMESTAMP`、PostgreSQL 不需要转换函数）并输入起始/终止日期（`yyyymmdd`），按日生成分区（分区名为上界减一天，如 `p20260920 VALUES LESS THAN (20260921)`），终止日期留空时以 `pmax VALUES LESS THAN (MAXVALUE)` 兜底（PostgreSQL 为 `default` 分区，且分区定义为独立的 `PARTITION OF` 语句）；分区区块提供快捷推算：输入基准日期、粒度（D/M/Y）与前置/后置分区数，点击“推算起止日期”自动填充起始（基准向前推前置数）与终止（基准向后推后置数，月/年不足同日时对齐月末）日期输入框，推算后仍可手工修改；弹窗中的分片/分区选择仅作用于本次生成的 DDL，不写回模型；生成结果会用 sqlglot 按对应方言做 SQL 语法校验并显示校验结论（sqlglot 为可选依赖，未安装时提示跳过；分布式方言的分布子句与 Oracle 的内联分区子句先剥离、`RANGE COLUMNS` 降级为 `RANGE` 做语法近似校验）；表若通过 extension 引用公共字段表，则在字段与物理索引之间按引用顺序逐个展示“公共字段表”小节（含各公共表的字段明细，标题可点击跳转，未解析引用灰色显示）；并展示物理索引、ODB 索引、序列；交易另展示流程编排（mermaid 纵向流程图：`method` 直调步骤按序串联；`case` 为菱形判断节点，各 `when` 分支从 case 分叉、以条件名命名并纵向串联各自的服务调用，分支结束后汇合到下一步骤；mermaid 随包内置、离线可用，加载失败时自动降级为缩进列表）；错误码按分组展示明细，表头为 `id, 类型, 错误码, 参数, message`（错误码列为完整 fullId，参数为该错误码的 parameter 子节点）。数据字典/复合类型详情的数据项表格中，`字典ID` 在 ref 缺省时展示数据项自身 fullId（如 `BpDict.A.addr`）；凡值符合模型 fullId 形态（大写开头点分，如 `BaseType.U_ADDR`、`BpDict.E.entp_scale`）的属性（含字段表格中的 `类型`/`字典ID` 列与详情属性区）均渲染为可点击链接跳转到目标详情；各元数据模型详情在“引用（出）”之前展示“原始 XML 片段”（文件级节点除外：命名SQL文件、数据字典文件、错误码文件不展示）：按索引记录的来源路径按需读取源 XML 并定位到该节点（不写入 SQLite），内容为该节点的原始 XML（含全部子结构）。已解析引用渲染为可点击链接跳转，未解析引用灰色显示原始目标。详情面板顶部提供“← 返回上一级”按钮，沿跳转历史逐级返回；切换左侧页面时历史清空。详情属性过滤 `xsi:` 等命名空间属性，避免撑宽页面。
...
5. **分片表展开与表分片类型是两个概念**：表属性 `sharding>1` 时按原模板行为展开为 `表名_0..表名_N-1` 多张物理表（分布子句对每张展开表重复）；workbench 弹窗的“表分片类型”（分片表/广播表/复制表）是分布式数据库的表尾分布子句选择，仅作用于本次生成的 DDL。CLI `ddl-gen` 不提供分片/分区参数，对 TDSQL/GoldenDB 按单表生成
